          ...
          if (_isString || _isRef) {
            const doSet = () => {
              if (rawRef.f) {
                ...
                warn2("Invalid template ref type:", ref, `(${typeof ref})`);
              }
            };
            if (value) {
              doSet.id = -1;
...
- For vite: pass it via @vitejs/plugin-vue options. See https://github.com/vitejs/vite-plugin-vue/tree/main/packages/plugin-vue#example-for-passing-options-to-vuecompiler-sfc`;
  ...
  function resetStoreState(store, state, hot) {
    ...
    store._state = (0, vue_runtime_esm_bundler_exports.reactive)({
      data: state
    });
    if (store.strict) {
  ...
  function enableStrictMode(store) {
    (0, vue_runtime_esm_bundler_exports.watch)(function() {
  ...
  Store.prototype.watch = function watch$1(getter, cb, options) {
    ...
    return (0, vue_runtime_esm_bundler_exports.watch)(function() {
  ...
  // ../qyass/qyass/public/js/frappe/views/kanban/kanban_board.bundle.js
  ...
  (function() {
    ...
    frappe.views.KanbanBoard = function(opts) {
      var self2 = {};
      self2.wrapper = opts.wrapper;
      self2.cur_list = opts.cur_list;
      self2.board_name = opts.board_name;
      self2.board_perms = self2.cur_list.board_perms;
      self2.update = function(cards) {
        opts.cards = cards;
        if (self2.wrapper.find(".kanban").length > 0 && self2.cur_list.start !== 0) {
      ...
      function prepare() {
        self2.$kanban_board = self2.wrapper.find(".kanban");
        if (self2.$kanban_board.length === 0) {
          self2.$kanban_board = $(frappe.render_template("kanban_board"));
          self2.$kanban_board.appendTo(self2.wrapper);
        }
        self2.$filter_area = self2.cur_list.$page.find(".active-tag-filters");
        bind_events();
        setup_sortable();
      }
      function make_columns() {
        self2.$kanban_board.find(".kanban-column").not(".add-new-column").remove();
        var columns = store.state.columns;
        columns.filter(is_active_column).map(function(col) {
          frappe.views.KanbanBoardColumn(col, self2.$kanban_board, self2.board_perms);
        });
      }
      ...
      function setup_sortable() {
        if (!self2.board_perms.write)
          return;
        var sortable = new Sortable(self2.$kanban_board.get(0), {
          group: "columns",
          animation: 150,
      ...
      function bind_add_column() {
        if (!self2.board_perms.write) {
          self2.$kanban_board.find(".add-new-column").remove();
          return;
        }
        var $add_new_column = self2.$kanban_board.find(".add-new-column"), $compose_column = $add_new_column.find(".compose-column"), $compose_column_form = $add_new_column.find(".compose-column-form").hide();
      ...
      function bind_clickdrag() {
        ...
        let startX;
        let scrollLeft;
        let draggable = self2.$kanban_board[0];
      ...
      function setup_restore_columns() {
        ...
        var $dropdown = $(
          "<div class='dropdown pull-right'><a class='text-muted dropdown-toggle' data-toggle='dropdown'><span class='dropdown-text'>" + __("Archived Columns") + "</span><i class='caret'></i></a><ul class='dropdown-menu'>" + options + "</ul></div>"
        );
      ...
      function show_empty_state() {
        ...
        if (empty_state) {
          self2.$kanban_board.find(".kanban-column").hide();
          self2.$kanban_board.find(".kanban-empty-state").show();
        } else {
          self2.$kanban_board.find(".kanban-column").show();
          self2.$kanban_board.find(".kanban-empty-state").hide();
        }
      }
      init();
      return self2;
    ...
    frappe.views.KanbanBoardColumn = function(column, wrapper, board_perms) {
      ...
      function make_dom() {
        ...
        self2.$kanban_cards = self2.$kanban_column.find(".kanban-cards");
      }
      function make_cards() {
        self2.$kanban_cards.empty();
        ...
        if (order) {
          ...
          filtered_cards.forEach(function(card) {
            if (order.indexOf(card.name) === -1) {
              frappe.views.KanbanBoardCard(card, self2.$kanban_cards);
            }
          });
          order.forEach(function(name) {
            if (!filtered_cards_names.includes(name))
              return;
            frappe.views.KanbanBoardCard(get_card(name), self2.$kanban_cards);
          });
        } else {
      ...
      function setup_sortable() {
        if (!frappe.model.can_write(store.state.doctype))
          return;
        Sortable.create(self2.$kanban_cards.get(0), {
          group: "cards",
          animation: 150,
      ...
      function bind_add_card() {
        var $wrapper = self2.$kanban_column;
      ...
      function bind_options() {
        if (!board_perms.write) {
          self2.$kanban_column.find(".column-options").remove();
          return;
        }
        self2.$kanban_column.find(".column-options .dropdown-menu").on("click", "[data-action]", function() {
          var $btn = $(this);
        ...
        get_column_indicators(function(indicators) {
          ...
          self2.$kanban_column.find(".column-options .dropdown-menu").append(html);
        });
      }
      init();
    };
    frappe.views.KanbanBoardCard = function(card, wrapper) {
      ...
      function make_dom() {
        ...
        const CARD_TEMPLATE = `<div class="kanban-card-wrapper {{ disable_click }}" data-name="{{encodeURIComponent(name)}}">
            ...
            `;
        if (card.doctype === "Organization Entry Records") {
          self2.$card = $(frappe.render(CARD_TEMPLATE, opts)).appendTo(wrapper);
        } else {
          self2.$card = $(frappe.render_template("kanban_card", opts)).appendTo(wrapper);
        }
        if (!frappe.model.can_write(card.doctype)) {
          self2.$card.find(".kanban-card-body").css("cursor", "default");
        }
      }
      function get_doc_content(card2) {
        var _a, _b;
        if (card2.doctype === "Organization Entry Records") {
          var fields = [];
          for (let field_name of cur_list.board.fields) {
            if (field_name === "element") {
              continue;
            }
            let field = ((_a = frappe.meta.docfield_map[card2.doctype]) == null ? void 0 : _a[field_name]) || frappe.model.get_std_field(field_name);
          ...
          var fields = [];
          for (let field_name of cur_list.board.fields) {
            let field = ((_b = frappe.meta.docfield_map[card2.doctype]) == null ? void 0 : _b[field_name]) || frappe.model.get_std_field(field_name);
            let label = cur_list.board.show_labels ? `<span>${__(field.label)}: </span>` : "";
            let value = frappe.format(card2.doc[field_name], field);
            fields.push(`
      ...
      function render_card_meta() {
        ...
        if (card.color && frappe.ui.color.validate_hex(card.color)) {
          ...
          self2.$card.find(".kanban-card .kanban-title-area").prepend($div);
        }
        self2.$card.find(".kanban-card-meta").empty().append(html).find(".kanban-assignments").append($assignees_group);
      }
      ...
      function show_assign_to_dialog(e) {
        e.preventDefault();
        e.stopPropagation();
        self2.assign_to = new frappe.ui.form.AssignToDialog({
          obj: self2,
          method: "frappe.desk.form.assign_to.add",
          doctype: card.doctype,
          docname: card.name,
          callback: function() {
            const users = self2.assign_to_dialog.get_values().assign_to;
            card.assigned_list = [...new Set(card.assigned_list.concat(users))];
            store.dispatch("update_card", card);
          }
        });
        self2.assign_to_dialog = self2.assign_to.dialog;
        self2.assign_to_dialog.show();
      }
      init();
    };
 ...
 */
/*! #__NO_SIDE_EFFECTS__ */
//# sourceMappingURL=kanban_board.bundle.MO5SROER.js.map
